...
/**
 * Formats a color as an OKLCH CSS string.
 * @param color - A culori Color object (typically Rgb)
 * @returns OKLCH CSS string like "oklch(0.5 0.1 180)" or "oklch(0.5 0.1 180 / 0.8)"
 */
export function formatOklch(color: Color): string {
  ...
  if (oklchColor.alpha !== undefined && oklchColor.alpha < 1) {
    const alpha = oklchColor.alpha.toFixed(4)
