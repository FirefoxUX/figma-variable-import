...
import { fetchFigmaAPI, FigmaAPIURLs } from './api.js'
...
export async function submitVDCollections(
  ...
  const uc = new UpdateConstructor(figmaTokens, fileId, figmaAccessToken)
...
export async function getFigmaCollections(
  ...
  const { meta } = await fetchFigmaAPI<GetLocalVariablesResponse>(
    url,
    figmaAccessToken,
  )
