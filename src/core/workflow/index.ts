...
class WorkflowLogger {
  ...
  private getJobInfo(data: WorkflowData): InfoBoxMessage | undefined {
    ...
    } else if (data.dryRun && 'stats' in data) {
      infoMessage = {
        type: 'note',
        message:
          'Dry run — no changes were submitted. Below is the changeset that would have been applied.',
      }
    } else if (!data.dryRun && 'stats' in data) {
      const { stats } = data
  ...
  private async createJobSlackMessage(
    ...
    const webookUrl =
      'stats' in data
        ? this.config.slackWebhookUrlSuccess
        : this.config.slackWebhookUrlFailure
    if (!webookUrl) {
      return
    }
